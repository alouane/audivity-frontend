...
class SubmitScript extends Component {
    ...
    submit = values => {
        ...
        //Init react draft wyswyg
        var introScript = "";
        var mainScript = "";
        var outroScript = "";
        var scriptNotes = "";

        if(values.introScript) values.introScript.blocks.forEach(function (element) {
            console.log(element.text);
            introScript += element.text + "\n"
        });

        if(values.mainScript) values.mainScript.blocks.forEach(function (element) {
            console.log(element.text);
            mainScript += element.text + "\n"
        });

        if(values.outroScript) values.outroScript.blocks.forEach(function (element) {
            console.log(element.text);
            outroScript += element.text + "\n"
        });

        if(values.scriptNotes) values.scriptNotes.blocks.forEach(function (element) {
            console.log(element.text);
            scriptNotes += element.text + "\n"
        });

        let message = "New Scripts Submitted\n\nURL: " + values.url + "\n\nTitle: " + values.title + "\n\nIntro Script: " + introScript + "\n\nMain Script: " + mainScript + "\n\nOutro Script: " + outroScript + "\n\nNotes: " + scriptNotes;
